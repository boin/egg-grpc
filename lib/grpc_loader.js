...
module.exports = app => {
  ...
  const defaults = {
    ...
    initializer(content, meta) {
      ...
      return grpc.loadPackageDefinition(require('@grpc/proto-loader').loadSync(meta.path, config.loadOpts));
    },
  };
    ...
    parse() {
      ...
      for (const item of items) {
        ...
        traverse(exports).forEach(function(proto) {
          ...
          if (!proto || !proto.name) return; // skip invalid values
  ...
  function camelize(input) {
    input = input.replace(/[_-][a-z]/gi, s => s.substring(1).toUpperCase());
